...
def gather_facts(target: str, from_branch: str, project: str, repository: str , username: str, token: str, bitbucket_host: str, title_and_description: str) -> list:
    """
    It gathers facts about the pull request
...
def create_pull_request(target: str, yes: bool, diff: bool) -> None:
    ...
    :param target: The branch you want to merge into
    :type target: str
    :param yes: If you want to skip the prompt and just create the pull request, set this to True
    :type yes: bool
    :param diff: bool
    :type diff: bool
    """
    ...
    header, reviewers = gather_facts(target, from_branch, project, repository , username, token, bitbucket_host, title_and_description)
    ...
    if yes or prompt("Proceed [y/n]").lower().strip() == 'y':
        ...
        if pull_request[0] == 201:
            richprint.console.print(f"Pull Request Created: {pull_request[1]['links']['self'][0]['href']}", highlight=True, style='bold green')
            id = pull_request[1]['links']['self'][0]['href'].split('/')[-1]
            copy_to_clipboard(pull_request[1]['links']['self'][0]['href'])
            richprint.str_print("Tip: Pull request url is copied to clipboard ('ctrl+v' to paste)", "dim white")
        elif pull_request[0] == 409:
            richprint.console.print(f"Message: {pull_request[1]['errors'][0]['message']}", highlight=True, style="bold red")
            richprint.console.print(f"Existing Pull Request: {pull_request[1]['errors'][0]['existingPullRequest']['links']['self'][0]['href']}", highlight=True, style='bold yellow')
            id = pull_request[1]['errors'][0]['existingPullRequest']['links']['self'][0]['href'].split('/')[-1]
